    ...
    try:
        ...
        # ── Text Only — transcript, strip timestamps ──────────────────────────
            ...
            for srt_file in out_path.glob("*.srt"):
                ...
                try:
                    lines = srt_file.read_text(encoding="utf-8", errors="replace").split("\n")
                    out_lines = []
                    for line in lines:
                        line = line.strip()
        ...
        if fmt in ("subtitles", "textonly"):
            ...
            vfiles = [f for f in media_files
                      if f.suffix.lower() not in {".srt", ".vtt", ".txt"}]
            vfiles.sort(key=lambda f: f.stat().st_size, reverse=True)
            main_file = vfiles[0] if vfiles else media_files[0]
...
def get_file(job_id, filename):
    ...
    @after_this_request
    def maybe_cleanup(response):
        try:
            with jobs_lock:
                fetched = jobs[job_id].get("fetched", 0) + 1
                jobs[job_id]["fetched"] = fetched
            if fetched >= total:
                shutil.rmtree(Path(job["out_path"]), ignore_errors=True)
                with jobs_lock:
                    jobs.pop(job_id, None)
                log("🗑️", f"[{job_id}] Cleaned up after {fetched} file(s)")
        except Exception:
            pass
        return response

    log("📤", f"[{job_id}] Sending {filename} to phone")
